    ...
    : m_cubeEdgeSize(edgeSize), m_meshData(meshData)
{
    ...
    m_commonBbox = m_meshData.getMeshComponents().front().m_tetrahedron.bbox();
    for (const auto &tetrahedronData : m_meshData.getMeshComponents())
        m_commonBbox += tetrahedronData.m_tetrahedron.bbox();
            ...
            for (short x{}; x < m_divisionsX; ++x)
            {
                for (short y{}; y < m_divisionsY; ++y)
                {
                    for (short z{}; z < m_divisionsZ; ++z)
                    {
                        ...
                        if (CGAL::do_overlap(cellBox, tetrahedronData.m_tetrahedron.bbox()))
                            m_tetrahedronCells[tetrahedronData.m_globalTetraId].emplace_back(x, y, z);
                    }
                }
            }
        }
...
GridIndex CubicGrid::getGridIndexByPosition(Point_cref point) const
{
    return {
...
bool CubicGrid::isInsideTetrahedronMesh(Point_cref point) const
{
    ...
    for (size_t i{}; i < tetrahedrons.size(); ++i)
        checks[i] = tetrahedrons.at(i).isPointInside(point);

    // If bitset contains at least one `true` - it means that point is inside the tetrahedron mesh.
    return checks.any();
}

TetrahedronDataVector CubicGrid::getTetrahedronsByGridIndex(GridIndex_cref index) const
{
    TetrahedronDataVector meshParams;
...
std::optional<size_t> CubicGrid::getContainingTetrahedron(ParticleTrackerMap_cref particleTracker,
