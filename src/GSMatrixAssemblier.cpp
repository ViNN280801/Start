...
Scalar getValueFromMatrix(Teuchos::RCP<TpetraMatrixType> const &matrix,
                          GlobalOrdinal row, GlobalOrdinal col) {
  try {
    ...
    TpetraMatrixType::nonconst_global_inds_host_view_type indices("ind",
                                                                  numEntries);
    ...
    WARNINGMSG(util::stringify("No entry found at [", row, "][", col,
                               "]. Returning ", -1ul << '\n'));
    return -1ul;
...
shards::CellTopology GSMatrixAssemblier::_getTetrahedronCellTopology() const {
  return shards::getCellTopologyData<shards::Tetrahedron<>>();
}

auto GSMatrixAssemblier::_getBasis() const {
  return Intrepid2::Basis_HGRAD_TET_Cn_FEM<DeviceType>(m_polynomOrder);
}
...
void GSMatrixAssemblier::_initializeCubature() {
  try {
    ...
    _countCubPoints =
        cubature->getNumPoints();         // Getting number of cubature points.
    _spaceDim = cubature->getDimension(); // Getting dimension (for tetrahedron,
                                          // obviously - 3D).

    // 1. Allocating memory for cubature points and weights.
    _cubPoints =
        DynRankView("cubPoints", _countCubPoints,
                    _spaceDim); // Matrix: _countCubPoints x Dimensions.
    _cubWeights =
        DynRankView("cubWeights", _countCubPoints); // Vector: _countCubPoints.
...
DynRankView GSMatrixAssemblier::_getTetrahedronVertices(
    MeshTetrahedronParamVector const &meshParams) const {
  try {
    DynRankView vertices("vertices", _countTetrahedra,
                         _tetrahedronVerticesCount, _spaceDim);
  ...
  WARNINGMSG("Returning empty multidimensional array with vertices of the all "
             "tetrahedrons from the mesh");
  return DynRankView();
}
...
DynRankView GSMatrixAssemblier::_computeTetrahedronBasisFunctionValues() {
  try {
    ...
    DynRankView basisFunctionValues("basisValues", _countBasisFunctions,
                                    _countCubPoints);
  ...
  WARNINGMSG("Returning empty multidimensional array which was intended for "
             "basis function values");
  return DynRankView();
}

DynRankView
GSMatrixAssemblier::_computeTetrahedronBasisFunctionValuesTransformed(
    MeshTetrahedronParamVector const &meshParams) {
  try {
    auto basisFunctionValues{_computeTetrahedronBasisFunctionValues()};

    DynRankView transformedBasisFunctionValues(
        "transformedBasisValues", _countTetrahedra, _countBasisFunctions,
        _countCubPoints);
    ...
    Intrepid2::CellTools<DeviceType>::mapToPhysicalFrame(
        transformedBasisFunctionValues, _cubPoints, vertices, cellTopology);
  ...
  WARNINGMSG("Returning empty multidimensional array which was intended for "
             "basis function values that are transformed to physical space");
  return DynRankView();
}

DynRankView GSMatrixAssemblier::_computeTetrahedronBasisFunctionGradients() {
  try {
    DynRankView basisGradients("basisGradients", _countBasisFunctions,
                               _countCubPoints, _spaceDim);
...
DynRankView GSMatrixAssemblier::_computeCellJacobians(
    MeshTetrahedronParamVector const &meshParams) {
  try {
    DynRankView jacobians("jacobians", _countTetrahedra, _countCubPoints,
                          _spaceDim, _spaceDim);
    ...
    Intrepid2::CellTools<DeviceType>::setJacobian(jacobians, _cubPoints,
                                                  vertices, cellTopology);
...
DynRankView
GSMatrixAssemblier::_computeInverseJacobians(DynRankView const &jacobians) {
  try {
    DynRankView invJacobians("invJacobians", _countTetrahedra, _countCubPoints,
                             _spaceDim, _spaceDim);
  ...
  WARNINGMSG("Returning empty multidimensional array which was intended for "
             "inversed cell jacobians");
  return DynRankView();
}

void GSMatrixAssemblier::_computeTetrahedronBasisFunctionGradientsTransformed(
    MeshTetrahedronParamVector const &meshParams) {
  try {
    m_basisFuncGrads =
        DynRankView("transformedBasisGradients", _countTetrahedra,
                    _countBasisFunctions, _countCubPoints, _spaceDim);
    ...
    Intrepid2::FunctionSpaceTools<DeviceType>::HGRADtransformGRAD(
        m_basisFuncGrads, invJacobians, basisGradients);
    ...
    size_t firstTetraID{std::get<0>(
        meshParams.front())}; // Local index is 0, global index for tetrahedra
                              // is the 1st element of the tuple.
    auto tetraNodesMap{Mesh::getTetrahedronNodesMap(m_meshfilename)};

    for (size_t localTetraId{}; localTetraId < meshParams.size();
         ++localTetraId) {
      auto globalNodeIds{tetraNodesMap.at(localTetraId + firstTetraID)};
      for (short localNodeId{}; localNodeId < _countBasisFunctions;
           ++localNodeId) {
        // As we have polynom order = 1, that all the values from the ∇φ in all
        // cub points are the same, so we can add only 1 row from each ∇φ.
        m_basisFuncGradientMap[localTetraId + firstTetraID].insert(
            std::make_pair(
...
DynRankView GSMatrixAssemblier::_computeLocalStiffnessMatrices(
    DynRankView const &basisGradients) const {
  try {
    DynRankView localStiffnessMatrices("localStiffnessMatrices",
                                       _countTetrahedra, _countBasisFunctions,
                                       _countBasisFunctions);
        ...
        for (int j{}; j < _countBasisFunctions; ++j)
          for (int qp{}; qp < _countCubPoints; ++qp) {
            ...
            for (short d{}; d < _spaceDim; ++d)
              gradDotProduct += basisGradients(tetraId, i, qp, d) *
                                basisGradients(tetraId, j, qp, d);
            localStiffnessMatrices(tetraId, i, j) +=
                gradDotProduct * _cubWeights(qp);
          }
...
std::vector<GSMatrixAssemblier::MatrixEntry>
GSMatrixAssemblier::_getMatrixEntries(
    DynRankView const &basisGradients,
  ...
  try {
    for (size_t tetraId{}; tetraId < globalNodeIndicesPerElement.size();
         ++tetraId) {
      auto const &nodeIndices{globalNodeIndicesPerElement.at(tetraId)};
      for (size_t localNodeI{}; localNodeI < _tetrahedronVerticesCount;
           ++localNodeI) {
        for (size_t localNodeJ{}; localNodeJ < _tetrahedronVerticesCount;
             ++localNodeJ) {
          for (int basisI{}; basisI < countBasisFuncsPerNode; ++basisI) {
            for (int basisJ{}; basisJ < countBasisFuncsPerNode; ++basisJ) {
              ...
              GlobalOrdinal globalRow{
                  nodeIndices[localNodeI] * countBasisFuncsPerNode + basisI},
                  globalCol{nodeIndices[localNodeJ] * countBasisFuncsPerNode +
                            basisJ};
  ...
  if (matrixEntries.empty())
    WARNINGMSG("Something went wrong while filling matrix entries - matrix "
               "entries are empty - there is no elements");

  return matrixEntries;
}

void GSMatrixAssemblier::_assemblyGlobalStiffnessMatrixHelper(
    DynRankView const &basisGradients,
    TetrahedronIndicesVector const &globalNodeIndicesPerElement) {
  try {
    // 1. Getting all matrix entries.
    auto matrixEntries{
        _getMatrixEntries(basisGradients, globalNodeIndicesPerElement)};
    ...
    for (auto const &rowEntry : graphEntries)
      numEntriesPerRow.at(m_map->getLocalElement(rowEntry.first)) =
          rowEntry.second.size();
    ...
    Teuchos::RCP<Tpetra::CrsGraph<>> graph{Teuchos::rcp(new Tpetra::CrsGraph<>(
        m_map, Teuchos::ArrayView<size_t const>(numEntriesPerRow.data(),
                                                numEntriesPerRow.size())))};
    for (auto const &rowEntries : graphEntries) {
      std::vector<GlobalOrdinal> columns(rowEntries.second.begin(),
                                         rowEntries.second.end());
      Teuchos::ArrayView<GlobalOrdinal const> colsView(columns.data(),
                                                       columns.size());
    ...
    for (auto const &entry : matrixEntries) {
      ...
      Teuchos::ArrayView<GlobalOrdinal const> colsView(
          std::addressof(entry.col), 1);
    ...
    ERRMSG(
        "Unknown error was occured while assemblying global stiffness matrix. "
        "Probably solution: decrease polynom order or desired accuracy");
  }
}

void GSMatrixAssemblier::_setBoundaryConditionForNode(LocalOrdinal nodeID,
                                                      Scalar value) {
  ...
  for (size_t i{}; i < numEntries; i++)
    values[i] =
        (indices[i] == nodeID)
            ? 1.0
            : 0.0; // Set diagonal value to specified value, other - to 0.
...
GSMatrixAssemblier::GSMatrixAssemblier(std::string_view mesh_filename,
  ...
  if (desiredCalculationAccuracy <= 0) {
    ...
    throw std::runtime_error(
        "Desired calculation accuracy can't be negative or equals to 0");
  }
...
void GSMatrixAssemblier::assembleGlobalStiffnessMatrix(
    std::string_view mesh_filename) {
  ...
  if (tetrahedronMesh.empty())
    throw std::runtime_error(
        util::stringify("Can't get mesh parameters from file ", mesh_filename));
  ...
  _assemblyGlobalStiffnessMatrixHelper(m_basisFuncGrads,
                                       globalNodeIndicesPerElement);
}

bool GSMatrixAssemblier::empty() const {
  return m_gsmatrix->getGlobalNumEntries() == 0;
}

Scalar GSMatrixAssemblier::getValueFromGSM(GlobalOrdinal row,
                                           GlobalOrdinal col) const {
  return getValueFromMatrix(m_gsmatrix, row, col);
}

void GSMatrixAssemblier::setBoundaryConditions(
    std::map<GlobalOrdinal, Scalar> const &boundaryConditions) {
  if (boundaryConditions.empty()) {
    WARNINGMSG("Boundary conditions are empty, check them, maybe you forgot to "
               "fill them");
    return;
  }

  if (empty()) {
    ERRMSG("Can't set boundary conditions. Matrix is uninitialized/empty, "
           "there are no any entries");
    return;
  }

  try {
    // 1. Ensure the matrix is in a state that allows adding or replacing
    // entries.
    ...
    for (auto const &[nodeInGmsh, value] : boundaryConditions) {
      for (int j{}; j < m_polynomOrder; ++j) {
        ...
        if (nodeID >= static_cast<GlobalOrdinal>(rows()))
          throw std::runtime_error(util::stringify(
              "Boundary condition refers to node index ", nodeID,
              ", which exceeds the maximum row index of ", rows() - 1, "."));

        _setBoundaryConditionForNode(
            nodeID, 1); // Maybe there is need to be 1, to have value = `value`
                        // in `x` vector, while solve Ax=b.
      }
    }
    ...
    ERRMSG("Unknown error was occured while trying to apply boundary "
           "conditions on global stiffness matrix");
  }
}
    ...
    for (int proc{}; proc < numProcs; ++proc) {
      if (myRank == proc) {
        ...
        for (size_t i{}; i < localNumRows; ++i) {
          ...
          TpetraMatrixType::nonconst_global_inds_host_view_type indices(
              "ind", numEntries);
          TpetraMatrixType::nonconst_values_host_view_type values("val",
                                                                  numEntries);
          size_t checkNumEntries{};

          m_gsmatrix->getGlobalRowCopy(globalRow, indices, values,
                                       checkNumEntries);
