...
void ParticleDynamicsProcessor::process(std::string_view config_filename,
                                        ...
                                        double gasConcentration)
{
    // Implement a fallback mechanism: first CUDA, then OpenMP, then standard multi-threading
    ...
    try
    {
        ...
        if (result != 0)
        {
            ERRMSG("Failed to execute 'nvidia-smi', try to install it: sudo apt-get install nvidia-cuda-toolkit");
            throw std::runtime_error("NVIDIA GPU not available");
        }
    ...
    // Fall back to standard C++ parallel execution if both CUDA and OpenMP failed
