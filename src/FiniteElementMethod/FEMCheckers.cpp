...
void FEMCheckers::checkDesiredAccuracy(short desired_accuracy)
{
    ...
    if (desired_accuracy > FEM_LIMITS_MAX_DESIRED_CALCULATION_ACCURACY)
        throw std::overflow_error("Desired calculation accuracy can't be greater than " + std::to_string(FEM_LIMITS_MAX_DESIRED_CALCULATION_ACCURACY) +
                                  ". Required range: [" + std::to_string(FEM_LIMITS_MIN_DESIRED_CALCULATION_ACCURACY) + "; " +
...
void FEMCheckers::checkPolynomOrder(short polynom_order)
{
    ...
    if (polynom_order > FEM_LIMITS_MAX_POLYNOMIAL_ORDER)
        throw std::overflow_error("Polynomial order can't be greater than " + std::to_string(FEM_LIMITS_MAX_POLYNOMIAL_ORDER) +
                                  ". Required range: [" + std::to_string(FEM_LIMITS_MIN_POLYNOMIAL_ORDER) + "; " +
