...
def check_dimtags(dimtags):
    ...
    if not dimtags:
        raise ValueError(
            f"{InternalLogger.pretty_function_details()}: Invalid dimtags: {dimtags}"
        )

    for dim, _ in dimtags:
        if dim != 3:
            raise ValueError(
                f"Invalid dimension {dim} in dimtags. Only dimension 3 is supported."
            )
...
def gmsh_cut_with_plane(input_dimtags, plane_dimtags):
    ...
    try:
        ...
        raise RuntimeError(
            f"Intersection operation cannot be performed with plane dimtags {plane_dimtags}"
        ) from e
