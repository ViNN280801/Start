...
class NumberCheckersTests(unittest.TestCase):
    ...
    def test_is_real_number(self):
        for _ in range(100000):
            ...
            if result != expected:
                print(f"test_is_real_number(): Failed value: {value}, Expected: {expected}, Got: {result}")
    ...
    def test_is_positive_real_number(self):
        for _ in range(100000):
            ...
            try:
                expected = float(value) >= 0 and not value in ['NaN', 'Infinity', '-Infinity']
            except ValueError:
                expected = False
            result = is_positive_real_number(value)
            if result != expected:
                print(f"test_is_positive_real_number(): Failed value: {value}, Expected: {expected}, Got: {result}")
    ...
    def test_is_positive_natural_number(self):
        for _ in range(100000):
            ...
            if result != expected:
                print(f"test_is_positive_natural_number(): Failed value: {value}, Expected: {expected}, Got: {result}")
