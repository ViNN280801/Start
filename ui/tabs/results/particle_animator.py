...
class ParticleAnimator:
    ...
    def __init__(
        ...
        self.active_particles = (
            set()
        )  # Tracks which particles are currently active in the visualization
    ...
    def load_config_from_tab(self):
        ...
        if self.config_tab is None:
            self.log_console.printWarning(
                "Config tab not available for direct configuration access"
            )
            return False
        ...
        try:
            ...
            if (
                hasattr(self.config_tab, "time_step_input")
                and self.config_tab.time_step_input.text()
            ):
                time_step_text = self.config_tab.time_step_input.text()
                time_step_unit = (
                    self.config_tab.time_step_units.currentText()
                    if hasattr(self.config_tab, "time_step_units")
                    else "s"
                )
                self.time_step = self.config_tab.converter.to_seconds(
                    time_step_text, time_step_unit
                )

            # Get simulation time from ConfigTab (converting to seconds)
            if (
                hasattr(self.config_tab, "simulation_time_input")
                and self.config_tab.simulation_time_input.text()
            ):
                sim_time_text = self.config_tab.simulation_time_input.text()
                sim_time_unit = (
                    self.config_tab.simulation_time_units.currentText()
                    if hasattr(self.config_tab, "simulation_time_units")
                    else "s"
                )
                self.total_simulation_time = self.config_tab.converter.to_seconds(
                    sim_time_text, sim_time_unit
                )

            # Check if sputtering is enabled
            if hasattr(self.config_tab, "sputtering_checkbox"):
                self.is_sputtering_mode = (
                    self.config_tab.sputtering_checkbox.isChecked()
                )

            self.log_console.printInfo(
                f"Loaded configuration from UI: Time Step={self.time_step}s, "
    ...
    def group_particles_by_spawn_time(self, particles_movement):
        ...
        sorted_particles = sorted(
            particles_movement.items(), key=lambda x: len(x[1]), reverse=True
        )

        # Estimate particles per group based on total particles and time steps
        particles_per_group = max(
            1,
            min(
        ...
        for i in range(0, len(sorted_particles), particles_per_group):
            ...
            self.particles_spawn_groups[current_group] = {
                pid: movements for pid, movements in group_particles
            }
            current_group += 1
        ...
        self.log_console.printInfo(
            f"Grouped particles into {len(self.particles_spawn_groups)} spawn time groups"
        )
        return True
    ...
    def animate_particle_movements(self, particles_movement):
        ...
        if self.is_sputtering_mode and self.group_particles_by_spawn_time(
            particles_movement
        ):
            ...
            if self.parent:
                ...
                msg_box.setText(
                    "Sputtering process detected. Use continuous sputtering animation?"
                )
        ...
        if self.is_sputtering_mode:
            ...
            max_trajectory_length = max(
                len(movements) for movements in particles_movement.values()
            )
            self.max_iterations = max_trajectory_length * self.FPS
            self.log_console.printInfo(
                f"Using sputtering animation mode: {len(self.particles_spawn_groups)} groups, "
        ...
        self.particle_id_to_index = {
            pid: i for i, pid in enumerate(self.particles_movement.keys())
        }
    ...
    def update_animation(self):
        ...
        if (
            hasattr(self, "particle_points")
            and self.particle_points is not None
            and hasattr(self, "particles_movement")
        ):
                ...
                if self.is_sputtering_mode:
                    ...
                    if particle_id not in self.active_particles:
                        ...
                        if len(movements) > 0:
                            ...
                            self.particle_points.SetPoint(
                                particle_index, [first_pos.x, first_pos.y, first_pos.z]
                            )
                        continue
                    ...
                    spawn_group = next(
                        (
                            g
                            for g, particles in self.particles_spawn_groups.items()
            ...
            if (
                hasattr(self, "particle_polydata")
                and self.particle_polydata is not None
            ):
        ...
        try:
            ...
            for particle_id, movements in data.items():
                ...
                particles_movement[particle_id] = [
                    PointForTracking(movement["x"], movement["y"], movement["z"])
