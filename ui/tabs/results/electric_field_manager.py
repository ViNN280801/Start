...
from vtk import (
    vtkPoints, vtkCellArray, vtkPolyData,
    vtkUnsignedCharArray, vtkArrowSource,
...
class ElectricFieldManager:
    ...
    def _process_pos_file(self, filename):
        ...
        points = vtkPoints()
        vectors = vtkCellArray()
        vector_magnitudes = []

        for (x, y, z), (vx, vy, vz) in zip(points_data, vectors_data):
            points.InsertNextPoint(x, y, z)
            vectors.InsertNextCell(2)
            vectors.InsertCellPoint(points.GetNumberOfPoints() - 1)
            vectors.InsertCellPoint(points.GetNumberOfPoints())
        ...
        poly_data.SetLines(vectors)

        vector_colors = vtkUnsignedCharArray()
        vector_colors.SetNumberOfComponents(3)
        vector_colors.SetName("Colors")

        for magnitude in vector_magnitudes:
            rgb = [0.0, 0.0, 0.0]
            color = [int(c * 255) for c in rgb]
            vector_colors.InsertNextTuple3(*color)

        poly_data.GetPointData().SetScalars(vector_colors)
