...
class GeometryManager:
    ...
    def delete_by_actor(search_actor: vtkActor):
        ...
        for index, (obj, actor, dimtags) in enumerate(GeometryManager.geometries):
            if actor == search_actor:
                ...
                return
        raise ValueError(
            f"Failed to delete object by actor: Actor {search_actor} not found in the geometries list."
        )
    ...
    def delete_by_dimtags(search_dimtags):
        ...
        for index, (obj, actor, dimtags) in enumerate(GeometryManager.geometries):
            if dimtags == search_dimtags:
                ...
                return
        raise ValueError(
            f"Failed to delete object by actor: Dimtags {search_dimtags} not found in the geometries list."
        )
    ...
    def remove(vtkWidget, renderer, actor_to_remove, needResetCamera):
        try:
            ...
            GeometryCreator.remove(
                vtkWidget, renderer, actor_to_remove, needResetCamera, dimtags_to_remove
            )
    ...
    def create_helper(obj_type, object, actor, tag) -> vtkActor:
        ...
        if obj_type == "point":
            ...
            if not isinstance(tag, list):
                raise TypeError(
                    f"{InternalLogger.pretty_function_details()}: For the line type variable `tag` must be `list`"
                )
            ...
            raise ValueError(
                f"{InternalLogger.pretty_function_details()} doesn't have object type [{obj_type}]"
            )
    ...
    def subtract_(first_actor, second_actor, first_dimtags, second_dimtags):
        return GeometryManipulator.subtract(
            first_actor, second_actor, first_dimtags, second_dimtags
        )

    @staticmethod
    def combine_(first_actor, second_actor, first_dimtags, second_dimtags):
        return GeometryManipulator.combine(
            first_actor, second_actor, first_dimtags, second_dimtags
        )

    @staticmethod
    def intersect_(first_actor, second_actor, first_dimtags, second_dimtags):
        return GeometryManipulator.intersect(
            first_actor, second_actor, first_dimtags, second_dimtags
        )
    ...
    def transform_general(
        ...
        if transform_name == "move":
            ...
            raise ValueError(
                f"Failed to perform transformation with name {transform_name}. There is no such transformation"
            )

    @staticmethod
    def operation_helper(
        operation: str, first_actor: vtkActor, second_actor: vtkActor
    ) -> vtkActor:
        from logger.internal_logger import InternalLogger
        ...
        try:
            ...
            if operation == "subtract":
                out_actor, out_dimtags = GeometryManipulator.subtract(
                    first_actor, second_actor, first_dimtags, second_dimtags
                )
                prefix = "subtracted"
            elif operation == "combine":
                out_actor, out_dimtags = GeometryManipulator.combine(
                    first_actor, second_actor, first_dimtags, second_dimtags
                )
                prefix = "combined"
            elif operation == "intersect":
                out_actor, out_dimtags = GeometryManipulator.intersect(
                    first_actor, second_actor, first_dimtags, second_dimtags
                )
                prefix = "intersected"
            else:
                raise ValueError(
                    f"{InternalLogger.pretty_function_details()} doesn't support operation [{operation}]"
                )
            ...
            raise ValueError(
                f"{InternalLogger.pretty_function_details()} can't perform operation {operation}. Reason: {e}"
            )
    ...
    def cross_section(actor, point1, point2, axis):
        ...
        out_actors, out_dimtags = GeometryManipulator.cross_section(
            actor, vtkplane, actor_dimtags, plane_dimtags
        )

        if len(out_actors) != 2 or len(out_dimtags) != 2:
            raise ValueError(
                f"Can't create cross-section. In result got <{len(out_actors)}> visual objects and <{len(out_dimtags)}> geometrical objects"
            )
    ...
    def mesh(filename: str, mesh_dim: int, mesh_size: float) -> bool:
        ...
        try:
            ...
            success_output = "Successfully meshed objects:\n" + "\n".join(
                [f"{i+1}) {obj}" for i, obj in enumerate(created_objects)]
            )
