...
from util import (
    rad_to_degree,
    extract_transform_from_actor,
    calculate_thetaPhi,
    calculate_thetaPhi_with_angles,
)
from dialogs import *
...
class ParticleSourceManager:
    ...
    def save_point_particle_source_to_config(self):
        try:
            ...
            if not config_file:
                QMessageBox.warning(
                    self.geditor,
                    "Saving Particle Source as Point",
            ...
            self.particle_source_dialog.accepted_signal.connect(
                lambda params: self.handle_particle_source_point_accepted(
            ...
            QMessageBox.warning(
                self.geditor, "Particle Source", f"Error defining particle source. {e}"
            )
            return None
    ...
    def update_config_with_particle_source(
        self, particle_params, surface_and_normals_dict
    ):
        config_file = self.config_tab.config_file_path
        if not config_file:
            QMessageBox.warning(
                self.geditor, "Configuration File", "No configuration file selected."
            )
            return
        ...
        for arrow_address, values in surface_and_normals_dict.items():
            ...
            coord_key = (
                f"{cell_center[0]:.2f}, {cell_center[1]:.2f}, {cell_center[2]:.2f}"
            )
            config_data["ParticleSourceSurface"][new_surface_row]["BaseCoordinates"][
                coord_key
            ] = normal
    ...
    def handle_particle_source_point_accepted(
        self, particle_params, config_file, config_data, theta, phi, base_coords
    ):
        try:
            ...
            self.log_console.printInfo(
                f"Successfully written coordinates of the particle source:\n"
                f"Base: {base_coords}\n"
            ...
            QMessageBox.warning(
                self.geditor, "Particle Source", f"Error saving particle source. {e}"
            )
            return None
    ...
    def get_particle_source_base_coords(self):
        if not self.particleSourceArrowActor or not isinstance(
            ...
            if method_dialog.exec_() == QDialog.Accepted:
                ...
                if method == "manual":
                    ...
                    self.expansion_angle_dialog = ExpansionAngleDialogNonModal(
                        self.vtkWidget, self.renderer, self.geditor
                    )
    ...
    def on_arrow_properties_accepted(self, properties):
        ...
        self.expansion_angle_dialog.accepted_signal.connect(
            lambda thetaMax: self.handle_theta_signal_with_thetaPhi(
                x, y, z, thetaMax, theta, phi
            )
        )
        self.expansion_angle_dialog.show()
        ...
        try:
            ...
            if thetaMax > pi / 2.0:
                self.log_console.printWarning(
                    f"The θ angle exceeds 90°, so some particles can distribute in the opposite direction\nθ = {thetaMax} ({thetaMax * 180. / pi}°)"
                )
            self.log_console.printInfo(
                f"Successfully assigned values to the expansion angle and calculated φ angle\nθ = {thetaMax} ({thetaMax * 180. / pi}°)\nφ = {phi} ({phi * 180. / pi}°)\n"
            )
