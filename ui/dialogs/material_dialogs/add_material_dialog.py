        ...
        try:
            ...
            if file_path:
                ...
                for i in range(self.materials_combobox.count()):
                    ...
                    components = dict(
                        [comp.split(": ") for comp in components_str.split(", ")]
                    )
                    # Convert percentages to floats
                    components = {k: float(v[:-1]) for k, v in components.items()}
                ...
                with open(file_path, "r") as file:
                    ...
                    for material in materials:
                        for name, components in material.items():
                            ...
                            if name in existing_names:
                                ...
                                self.materials_combobox.addItem(display_name)
                                self.materials_combobox.setItemData(
                                    self.materials_combobox.count() - 1,
                                    hint,
                                    Qt.ToolTipRole,
                                )
                    ...
                    if name_conflicts:
                        QMessageBox.warning(
                            self,
                            "Name Conflicts",
                            "These materials have conflicting names:\n"
                            + "\n".join(name_conflicts),
                        )
                    ...
                    if composition_conflicts:
                        QMessageBox.warning(
                            ...
                            "These materials have conflicting compositions:\n"
