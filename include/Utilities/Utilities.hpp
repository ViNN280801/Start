...
#define ERRMSG_ABS_PATH(desc)                                                  \
  std::cerr << std::format(                                                    \
      "\033[1;31mError:\033[0m\033[1m {}: {}({} line): {}: "                   \
      "\033[1;31m{}\033[0m\033[1m\n",                                          \
      util::getCurTime(), std::source_location::current().file_name(),         \
      std::source_location::current().line(), COMMON_PRETTY_FUNC, desc);
#define LOGMSG_ABS_PATH(desc)                                                  \
  std::clog << std::format(                                                    \
      "Log: {}: {}({} line): {}: {}\n", util::getCurTime(),                    \
      std::source_location::current().file_name(),                             \
      std::source_location::current().line(), COMMON_PRETTY_FUNC, desc);
#define EXTRACT_FILE_NAME(filepath)                                            \
  std::filesystem::path(std::string(filepath).c_str()).filename().string()
#define ERRMSG(desc)                                                           \
  std::cerr << std::format(                                                    \
      "\033[1;31mError:\033[0m\033[1m {}: {}({} line): {}: "                   \
      "\033[1;31m{}\033[0m\033[1m\n",                                          \
...
#define WARNINGMSG(desc)                                                       \
  std::cerr << std::format(                                                    \
      "\033[1;33mWarning:\033[0m\033[1m {}: {}({} line): {}: {}\n",            \
      util::getCurTime(),                                                      \
      EXTRACT_FILE_NAME(std::source_location::current().file_name()),          \
      std::source_location::current().line(), COMMON_PRETTY_FUNC, desc);
...
namespace util {
/**
 * @brief Concept that specifies all types that can be convert to
 * "std::string_view" type For example, "char", "const char *", "std::string",
 * etc.
 ...
 * For example, "%Y-%m-%d %H:%M:%S" for date and time in YYYY-MM-DD HH:MM:SS
 * format.
 * @param format The format string compatible with std::put_time. Defaults to
 * "%H:%M:%S".
 */
 ...
 */
template <Printable... Args> std::string stringify(Args &&...args) {
...
/// @brief Helper function to recieve string representation of the particle
/// type.
