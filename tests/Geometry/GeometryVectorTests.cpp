...
TEST(GeometryVectorConceptsTest, ValidNumericTypes)
{
...
TEST(GeometryVectorConceptsTest, InvalidNumericTypes)
{
...
TEST(GeometryVectorTraitsTest, ValidNumericTypes)
{
    // Test that valid numeric types (e.g., int, double) work correctly
...
TEST(GeometryVectorTraitsTest, InvalidNumericTypes)
{
...
TEST_F(GeometryVectorTest, DefaultConstructor)
{
    PositionVector vec;
...
TEST_F(GeometryVectorTest, ParameterizedConstructor)
{
...
TEST_F(GeometryVectorTest, AssignmentOperator)
{
    PositionVector vec;
    vec = 5.0;
...
TEST_F(GeometryVectorTest, CreateCoordinates)
{
...
TEST_F(GeometryVectorTest, Module)
{
    PositionVector vec(3.0, 4.0, 12.0);
    EXPECT_NEAR(vec.module(), 13.0, 0.0001);
}

TEST_F(GeometryVectorTest, Distance)
{
...
TEST_F(GeometryVectorTest, Clear)
{
...
TEST_F(GeometryVectorTest, IsNull)
{
    PositionVector vec1;
...
TEST_F(GeometryVectorTest, IsParallel)
{
    PositionVector vec1(1.0, 2.0, 3.0);
...
TEST_F(GeometryVectorTest, IsOrthogonal)
{
...
TEST_F(GeometryVectorTest, UnaryMinusOperator)
{
    PositionVector vec(1.0, 2.0, 3.0);
    GeometryVector negatedVec{-vec};
    EXPECT_DOUBLE_EQ(negatedVec.getX(), -1.0);
    EXPECT_DOUBLE_EQ(negatedVec.getY(), -2.0);
    EXPECT_DOUBLE_EQ(negatedVec.getZ(), -3.0);
}

TEST_F(GeometryVectorTest, SubtractionOperator)
{
...
TEST_F(GeometryVectorTest, AdditionOperator)
{
    PositionVector vec1(4.0, 5.0, 6.0);
    PositionVector vec2(1.0, 2.0, 3.0);
    PositionVector result{vec1 + vec2};
...
void testDotProduct()
{
    PositionVector vec1(2.0, 3.0, 4.0),
        vec2(3.0, 4.0, 5.0);
    [[maybe_unused]] double dotProdResult = vec1 * vec2,
...
void testCrossProduct()
{
    ...
    GeometryVector crossProdResult{vec1.crossProduct(vec2)};
...
void testRandomOperations(int test_count = 1'000'000)
{
    for (int i{}; i < test_count; ++i)
    {
        ...
        GeometryVector additionResult{vec1 + vec2},
            subtractionResult{vec1 - vec2};
        [[maybe_unused]] double dotProductResult = vec1 * vec2;
        GeometryVector crossProductResult{vec1.crossProduct(vec2)};
        [[maybe_unused]] double moduleResultVec1 = std::sqrt(vec1.getX() * vec1.getX() +
                                                             vec1.getY() * vec1.getY() +
                                                             vec1.getZ() * vec1.getZ()),
...
TEST_F(GeometryVectorTest, SubtractionOperatorWithValue)
{
    PositionVector vec(4.0, 5.0, 6.0);
    double value{2.0};
...
TEST_F(GeometryVectorTest, AdditionOperatorWithValue)
{
    PositionVector vec(4.0, 5.0, 6.0);
    double value{2.0};
    PositionVector result{vec + value};
...
TEST_F(GeometryVectorTest, FriendAdditionOperatorWithValue)
{
    PositionVector vec(4.0, 5.0, 6.0);
    double value{2.0};
...
TEST_F(GeometryVectorTest, ScalarMultiplication)
{
    PositionVector vec(2.0, 3.0, 4.0);
    double scalar{2.0};
...
TEST_F(GeometryVectorTest, FriendScalarMultiplication)
{
    PositionVector vec(2.0, 3.0, 4.0);
    double scalar{2.0};
...
TEST_F(GeometryVectorTest, DotProduct)
{
    PositionVector vec1(2.0, 3.0, 4.0);
    PositionVector vec2(3.0, 4.0, 5.0);
...
TEST_F(GeometryVectorTest, CrossProduct)
{
    PositionVector vec1(1.0, 2.0, 3.0);
    PositionVector vec2(4.0, 5.0, 6.0);
...
TEST_F(GeometryVectorTest, DivisionOperator)
{
    PositionVector vec(6.0, 8.0, 10.0);
    double divisor{2.0};
...
TEST_F(GeometryVectorTest, DivisionByZero)
{
    PositionVector vec(6.0, 8.0, 10.0);
    double divisor{};
...
TEST_F(GeometryVectorTest, ComparisonsOperator)
{
    PositionVector vec1(2.0, 3.0, 4.0);
    PositionVector vec2(2.0, 3.0, 4.0);
    PositionVector vec3(1.0, 3.0, 4.0);
...
TEST_F(GeometryVectorTest, OutputStreamOperator)
{
    PositionVector vec(2.0, 3.0, 4.0);
    std::ostringstream oss;
    oss << vec;
...
TEST_F(GeometryVectorTest, InputStreamOperator)
{
    PositionVector vec;
    std::istringstream iss("5 6 7");
    iss >> vec;
